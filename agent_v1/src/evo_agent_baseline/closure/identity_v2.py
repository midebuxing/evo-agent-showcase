...
OPEN_REASON_ORDER: dict = {
    # 2026-08-05 #33 保护闸新增，取新最大值（不动既有档位 ⇒ 既有 merge 逐字节不变）。
    # 排最高的理由：本码是**可解封**的——对专业审查员而言它带一个明确行动
    # （核实该事件是否真实发生），比下面那些「永久不能确立 / 缺一条事实」更该先看到。
    # ⚠️ 同「丁」路那条的留痕要求：取最高档只证明不扰动旧行为，不证明最高档本身对；
    # 它会遮蔽同键上的 `diagnostic_binding_not_valid_evidence` 等码。
    # 影响面：翻转集 22 行全部走同一码、同键竞争场景少，评估为小并接受。
